...
from drss_analysis import (DRSS, DynamicSoftSet, ALPHA_MULT_DEFAULT, BETA_MULT_DEFAULT,
                           ALPHA_MULT_LEGACY, BETA_MULT_LEGACY)
...
class LatentRiskBenchmark:
    ...
    def __init__(self, n_U: int = 200, n_Xstar: int = 60, n_E: int = 12,
                 ...
                 avail_persistence: float = 0.9, max_active: int | None = None,
                 seed: int | None = None):
        self.n_U = n_U
        ...
        # Optional cap on |A_t|: at most `max_active` parameters online per
        # slice (used for the active-set-size sensitivity of Section 12.3).
    ...
    def generate(self) -> Tuple[DynamicSoftSet, set, np.ndarray]:
        ...
        for t in range(1, self.n_T + 1):
            ...
            for e in range(self.n_E):
                ...
                if online[e]:
                    params[e] = set(base[e])
            if self.max_active is not None and len(params) > self.max_active:
                keep = self.rng.choice(sorted(params), size=self.max_active, replace=False)
                params = {int(e): params[int(e)] for e in keep}
            mappings[t] = params
...
def drss_scores(dss: DynamicSoftSet, X_star: set,
                alpha_mult: float = ALPHA_MULT_DEFAULT,
                beta_mult: float = BETA_MULT_DEFAULT) -> np.ndarray:
    """
    Element-wise DRSS score of Section 11.3, expressed with per-slice frequencies

        s(u) = f^L(u) + alpha~ . f^U(u) - beta~ . f^B(u),

    where (alpha~, beta~) = (alpha|U|, beta|U|).  The adopted default is
    (0.5, 0.25) (Table 8 / Table 9 of the manuscript).  The superseded default
    (1, 2) is available as (ALPHA_MULT_LEGACY, BETA_MULT_LEGACY) and is used only
    for the comparison rows of Tables 8 and 15.
    """
    drss = DRSS(dss)
    T = dss.T
    n_T = len(T)
