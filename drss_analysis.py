"""
=============================================================================
  Dynamic Rough Soft Sets (DRSS) — Core Framework and Analysis Code
=============================================================================

Section, table and figure numbers below refer to the revised manuscript.

  Part 1  — DRSS Core Framework (Sections 4–7)
      1.1  Dynamic Soft Approximation Space & Approximations (Definitions 4.1–4.2)
      1.2  Definability Classes (Definitions 4.5–4.6)
      1.3  Cross-Temporal Operators (Section 5)
      1.4  Operator Algebra (Section 6)
      1.5  Generalisation Theorems (Theorems 7.1–7.3)

  Part 2  — Entropy & Granularity (Section 9)
      2.1  Dynamic Soft Entropy (Definition 9.1)
      2.2  Monotonicity Under Refinement (Proposition 9.2)
      2.3  Temporal Granularity Drift (Definition 9.5)
      2.4  Entropy-Weighted Persistent Positive Region
      2.5  Liang–Shi vs DRSS Entropy Comparison (Section 9.1)

  Part 3  — Examples (Section 10)
      3.1  Abstract Example (Section 10.1, Table 6, Figure 4)
      3.2  Healthcare ICU Example (Section 10.2, Table 7, Figure 5)
      3.3  Boundary Non-Monotonicity Counterexample (Theorem 5.8)

  Part 4  — Algorithms (Section 11)
      4.1  Base Algorithm (Algorithm 1)
      4.2  Incremental Algorithm with Witness Counters (Algorithm 2)
      4.3  Score Function & Calibration (Section 11.3)
      NOTE: the wall-clock measurement reported in Section 12.4 is produced by
            ``drss_extended_analyses.wall_clock_protocol`` / ``wall_clock_scaling``;
            the measured benefit of Algorithm 2 is modest (see the manuscript).

  Part 5  — Synthetic Benchmark: LEGACY COUNTER-EXAMPLE ONLY
      ``DegenerateMixtureBenchmark`` is retained because the manuscript discusses
      why a mixture-sampled granulation cannot exercise a soft rough model.
      Every synthetic-benchmark number in the manuscript (Tables 8, 11, 12, 15
      and the sensitivity items of Section 12.3, Figures 6 and 7) is produced by
      ``drss_extended_analyses.py`` from ``drss_generative_model.LatentRiskBenchmark``.

  Part 6  — MIMIC-IV Experiment (Section 12.4)  [synthetic proxy only]
      MIMIC-IV is not redistributed; the proxy reproduces the pipeline, not the
      manuscript's clinical numbers (see README).

  Part 7  — Failure Mode Analysis (Section 13)
      ...
      7.4  Diagnostic Regression (Main Effects + Interaction) — Table 17

  Part 8  — DTRS + DRSS Integration (Section 14)
      8.1  Per-Slice Three-Way Decision Regions
      8.2  Persistent Three-Way Regions
      8.3  Worked ICU Example — Table 18
...
scale-free weights (alpha~, beta~) = (alpha|U|, beta|U|).  The adopted default is
(0.5, 0.25) (Table 8, Table 9).  The earlier default (1, 2) lies in the penalised
region beta~ > alpha~ and is retained ONLY as ``ALPHA_MULT_LEGACY, BETA_MULT_LEGACY``
so that the comparison rows of Tables 8 and 15 can be regenerated.  Every function
in this bundle uses the adopted default unless a legacy value is passed explicitly.
=============================================================================
"""
...
# ─────────────────────────────────────────────────────────────────────────────
# Score-weight defaults (Section 11.3, Tables 8 and 9 of the manuscript).
# (alpha~, beta~) = (alpha|U|, beta|U|).  Adopted default (0.5, 0.25); the
# superseded default (1, 2) is kept only for the comparison rows of Tables 8/15.
# ─────────────────────────────────────────────────────────────────────────────
ALPHA_MULT_DEFAULT = 0.5
BETA_MULT_DEFAULT  = 0.25
ALPHA_MULT_LEGACY  = 1.0
BETA_MULT_LEGACY   = 2.0


def default_alpha_beta(n_U: int) -> Tuple[float, float]:
    """Return the adopted (alpha, beta) = (0.5/|U|, 0.25/|U|) for a universe of size n_U."""
    return ALPHA_MULT_DEFAULT / n_U, BETA_MULT_DEFAULT / n_U
...
class DRSS:
    ...
    # ── 1.2  Pointwise approximations (Definition 4.2) ─────────────────────
    ...
    # ── 1.3  Definability classes (Definition 4.5) ──────────────────────────
    ...
    # ── Entropy-weighted persistent positive (Section 9.3) ──────────────────
...
def temporal_drift(dss: DynamicSoftSet, t1, t2) -> float:
    """ΔH(t,t') = |H_t(S_t) - H_{t'}(S_{t'})|  (Definition 9.5)"""
...
def run_abstract_example():
    """Reproduce all results reported in Section 10.1."""
    ...
    print(f"\nBoundary non-monotonicity (Theorem 5.8 counterexample):")
...
def run_icu_example():
    """Reproduce the ICU monitoring example of Section 10.2."""
    U = ["N", "AR", "C"]
    mappings = {
...
class DRSSAlgorithm:
    """
    Algorithm 1 (base) and Algorithm 2 (incremental) from Section 11.
    """
    ...
    def __init__(self, dss: DynamicSoftSet, X: set,
        ...
        self.X = frozenset(X)
        n = len(dss.U)
        a_def, b_def = default_alpha_beta(n)
        self.alpha = alpha if alpha is not None else a_def
        self.beta  = beta  if beta  is not None else b_def
        self.theta1 = theta1
        self.theta2 = theta2
...
def wall_clock_comparison(dss: DynamicSoftSet, X: set, n_reps: int = 50):
    """
    Quick wall-clock comparison between Alg 1 and Alg 2 (approximation update
    plus entropy and profile assembly, i.e. the same work in both variants).
    The protocol used for the numbers in Section 12.4 (repetitions, run-to-run
    range, peak memory, scaling series) is ``drss_extended_analyses.wall_clock_protocol``.
    """
...
# =============================================================================
# PART 5 — SYNTHETIC BENCHMARK (LEGACY counter-example; see drss_extended_analyses.py)
# =============================================================================

class DegenerateMixtureBenchmark:
    """
    Degenerate generative model, retained as a documented counter-example.

    Every granule is sampled as a mixture of X* members and non-members, so no
    granule is ever a subset of X*.  The soft lower approximation is then
    identically empty at every slice and the DRSS score degenerates to
    -frac_upper, which is anti-correlated with membership.

    Any granulation intended for a soft rough model must admit target-pure
    granules; see ``drss_generative_model.LatentRiskBenchmark`` for a
    construction that does.  Pinned by
    ``test_regression.py::test_mixture_generator_is_degenerate``.
    """
    ...
    def _pi(self, t: int) -> float:
        """Three-regime activation probability (Section 12.3)."""
        if t <= 20:
            return 0.4
        elif t <= 35:
            return 0.9
...
def baseline_B3_features_UNSOUND(dss: DynamicSoftSet, X_star: set) -> np.ndarray:
    """
    UNSOUND - do not use.  Scores each element by its membership in
    ``granule & X_star`` and therefore reads the target set directly, making the
    baseline an oracle (AUROC 1.0 by construction).  Use
    ``drss_generative_model.b3_scores``, which uses granule incidence only.
    Kept here only as a documented counter-example.
    """
...
def run_one_trial(rho: float, seed: int, alpha: float, beta: float) -> dict:
    ...
    for each method.
    """
    bench = DegenerateMixtureBenchmark(rho=rho, seed=seed)
    ...
    # B3
    pred_B3 = baseline_B3_features_UNSOUND(dss, X_star)
...
def run_synthetic_benchmark(n_runs: int = 100, rho: float = 0.75,
    ...
    if alpha is None or beta is None:
        a_def, b_def = default_alpha_beta(n_U)
        alpha = a_def if alpha is None else alpha
        beta  = b_def if beta  is None else beta
    rows = []
...
def print_synthetic_results(df: pd.DataFrame, title: str = ""):
    """Print a formatted results table (legacy helper; manuscript tables are printed by drss_extended_analyses.py)."""
...
def run_sensitivity_rho(n_runs: int = 100) -> pd.DataFrame:
    """LEGACY (degenerate generator). Table 12 is drss_extended_analyses.rho_sensitivity."""
...
def run_crosstemporal_ablation(n_runs: int = 100) -> dict:
    """LEGACY (degenerate generator). The ablation of Section 12.3 is drss_extended_analyses.crosstemporal_ablation."""
    errors_drss    = []
    errors_ablated = []
    for seed in range(n_runs):
        bench = DegenerateMixtureBenchmark(rho=0.75, seed=seed)
        dss, X_star, labels = bench.generate()
        n = len(dss.U)
        alpha, beta = default_alpha_beta(n)
...
# =============================================================================
# PART 6 — MIMIC-IV PROXY EXPERIMENT (Section 12.4)
# =============================================================================

class ClinicalProxyGenerator:
    """
    Synthetic proxy for MIMIC-IV clinical data (Section 12.4).
...
class ClinicalExperiment:
    """
    Full MIMIC-IV proxy experiment (Section 12.4).
    ...
    def run_single_cohort(self, seed: int) -> dict:
        ...
        t0 = time.perf_counter()
        I  = dss_te.T
        # element-wise score of Section 11.3 at the adopted default (0.5, 0.25)
        f_low = np.array([sum(1 for t in I if i in drss_te.lower(t, X_star)) / len(I)
                          for i in range(n_te)])
        f_up  = np.array([sum(1 for t in I if i in drss_te.upper(t, X_star)) / len(I)
                          for i in range(n_te)])
        scores_drss = (f_low + ALPHA_MULT_DEFAULT * f_up
                       - BETA_MULT_DEFAULT * (f_up - f_low))
...
class FailureModeAnalysis:
    """
    Section 13 illustrations on small random-granule universes.  LEGACY: the
    FM1 ablations and the diagnostic regression reported in the manuscript
    (Section 13.1, Table 17) are produced by drss_extended_analyses.py on the
    class-pure generator; the methods here are kept for exploration only.
    """
    ...
    def fm1_no_regime(self) -> dict:
        ...
        for seed in range(self.n_runs):
            ...
            dss    = DynamicSoftSet(U, mappings)
            labels = np.array([1 if u in X_star else 0 for u in U])
            scores = drss_predict(dss, X_star, *default_alpha_beta(n_U))
            pred   = (scores >= np.median(scores)).astype(int)
            errors_drss.append(1 - accuracy_score(labels, pred))
            pred_B1 = baseline_B1_features(dss, X_star)
    ...
    def diagnostic_regression(self, n_runs: int = 500, n_fit: int = 350,
                              seed: int = 42) -> Tuple[pd.DataFrame, dict]:
        """
        Table 17 (Section 13.2): diagnostic regression on synthetic runs.

        Protocol as stated in the manuscript: the response is the per-run error
        advantage of DRSS over B1 in percentage points; the predictors are the
        three label-free diagnostics (entropy variance, mean |A_t|, mean maximal
        granule share) and the single pre-specified interaction Var(H) x mean|A_t|;
        n_runs synthetic runs are split n_fit for fitting and the remainder held
        out; predictors are standardised on the fitting sample; R^2 is reported
        on the held-out runs for the main-effects model and the model with the
        interaction.  DRSS uses the adopted score weights (0.5, 0.25).
        """
        ...
        for run in range(n_runs):
            ...
            advantage = 100.0 * (err_B1 - err_drss)   # p.p., positive = DRSS wins

            H_vals   = [dynamic_soft_entropy(dss, t) for t in dss.T]
            at_sizes = [len(dss.active_params(t)) for t in dss.T]
            g_maxes  = [max((len(g) / n_U for g in dss.granules_at(t)), default=0.0)
                        for t in dss.T]
            rows.append({
                "run": run, "advantage": advantage,
        ...
        for col, label in [("var_H", "Var_t(H_t)"), ("mean_At", "Mean |A_t|"),
            ...
            corr.append({"diagnostic": label, "pearson_r": round(float(r), 3),
                         "p_value": float(pval)})
        ...
        Zf = (fit[full_cols] - mu) / sd
        Zh = (held[full_cols] - mu) / sd
        y_f, y_h = fit["advantage"].values, held["advantage"].values
        r2_main = LinearRegression().fit(Zf[main_cols], y_f).score(Zh[main_cols], y_h)
        r2_full = LinearRegression().fit(Zf[full_cols], y_f).score(Zh[full_cols], y_h)
        summary = {"n_runs": n_runs, "n_fit": n_fit, "n_heldout": n_runs - n_fit,
                   "r2_heldout_main": float(r2_main), "r2_heldout_interaction": float(r2_full)}
        print(f"\n  Held-out R^2 (main effects)   = {r2_main:.3f}")
        print(f"  Held-out R^2 (+ interaction)  = {r2_full:.3f}")
        return pd.DataFrame(corr), summary
...
def main():
    ...
    bench_timing = DegenerateMixtureBenchmark(n_U=100, n_T=20, seed=0)
    ...
    print(f"  Speedup:               {speedup:.2f}x  (quick check only; Section 12.4 "
          f"protocol is in drss_extended_analyses.py)")
    ...
    # The synthetic-benchmark tables of the manuscript (Tables 8, 11, 12, 15,
    # the sensitivity items of Section 12.3, Figures 6 and 7) are produced by
    # drss_extended_analyses.py from the class-pure LatentRiskBenchmark.  The
    # mixture generator kept in this file is a documented counter-example and
    # is NOT used for any manuscript number.
    print("\n[3/9] Synthetic benchmark: see `python drss_extended_analyses.py` "
          "(Tables 8, 11, 12, 15; Figures 6, 7).")
    print("[4/9] Calibration sweep (Table 8): see drss_extended_analyses.alpha_beta_grid.")
    print("[5/9] rho sensitivity (Table 12): see drss_extended_analyses.rho_sensitivity.")
    ...
    # FM1 ablations and the diagnostic regression of Table 17 are produced by
    # drss_extended_analyses.py on the class-pure generator (JSON keys
    # fm1_no_regime, fm1_static_granules, diagnostic_regression).  The
    # random-granule generators kept in FailureModeAnalysis are legacy
    # illustrations and are not used for any manuscript number.
    print("\n[7/9] Failure mode analysis: FM1 ablations and Table 17 are produced by "
          "drss_extended_analyses.py.")
    fma = FailureModeAnalysis(n_runs=30)
    r3  = fma.fm3_coarse_granules()
    print(f"  {r3['description']} (illustration): mean |Bnd| = {r3['mean_bnd_size']:.1f} "
    ...
    # (via another granule intersecting X^c). Remark 6.11 of the manuscript
    ...
    for t in dss_abs.T:
        ...
        if overlap:
            ...
            print(f"           (expected per Remark 6.11 for non-partition soft covers)")
    print("  ✓ Theorem 6.7 (Duality): verified; overlap cases confirm Remark 6.11 "
          "(non-partition covers produce L ∩ U_(X^c) ≠ ∅ as documented)")

    print("\n" + "=" * 70)
    print("  ALL ANALYSES COMPLETE")
    print("  Output figures: fig4_temporal_evolution.png, fig9_calibration.png,")
